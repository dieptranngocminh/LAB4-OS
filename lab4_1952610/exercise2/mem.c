...
void * best_fit_allocator(unsigned int size) {
	...
	while(current_region != NULL){
		if(size <= current_region->size){
			if(best == NULL || current_region->size < best->size){
				best = current_region;
				found = 1;
			}
		}
...
void * worst_fit_allocator(unsigned int size) {
	...
	int found = 0;
	struct mem_region * worst = NULL;
	struct mem_region * current_region = free_regions;
	
	while(current_region != NULL){
		if(size <= current_region->size){
			if(worst == NULL || current_region->size > worst->size){
				worst = current_region;
				found = 1;
			}
		}
